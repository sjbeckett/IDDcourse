...
# ╔═╡ b708e720-df26-11ef-3ef9-bf36dcaab341
begin
	using OrdinaryDiffEq, Plots, PlutoUI , PlutoTeachingTools, LaTeXStrings
	import PlutoUI: combine
end

# ╔═╡ ba205f44-ab9e-4772-af0d-f2a181054c43
md"""
...
# ╔═╡ 5e41c210-a88c-4f7e-9e98-d76003a0444a
md"""## Intervention:
...
# ╔═╡ 17be79b8-1ab4-44ee-b110-483b45c3dee1
md"""
...
# ╔═╡ 1662ae75-df62-4550-b148-fa73aa59caed
md"""
### 3) Large gatherings
...
# ╔═╡ 14386ba4-ac0d-43b5-a427-7c10842b1b0a
md"""
...
# ╔═╡ 5a58934f-0b80-432e-80bb-ddcffe21daf2
md"""
...
# ╔═╡ a5ad6b6a-bf35-44b4-9947-03500947477c
md"""
...
# ╔═╡ b43d3137-e094-46e8-8908-aacc89c69ca5
md"""
...
# ╔═╡ fef261c8-f808-449f-847a-73dc348dc93e
correct(md"""
# Takeaways
...
# ╔═╡ 7aea669a-a215-442d-b0fb-f22955dcb1a2
md"""
...
# ╔═╡ f7d42687-d3f7-4c57-8b43-d267173281fb
md"""
intial contact rate (per day) $(@bind c0 Slider(0:0.1:20,default=10,show_value=true))

intial probability of a contact resulting in transmission $(@bind q0 Slider(0:0.01:0.25,default=0.1,show_value=true))

time of intervention after awareness (days) $(@bind tc Slider(0:0.1:50,default=25,show_value=true))

changed contact rate (per day) $(@bind c1 Slider(0:0.1:20,default=10,show_value=true))

changed probability of a contact resulting in transmission $(@bind q1 Slider(0:0.01:0.25,default=0.1,show_value=true))

"""

# ╔═╡ 90e998a2-aa03-4c21-bf5a-237484521f18
function SEIRDmodel!(du,u,p,t)
	S, E, I, R, D = u
	β, ν, γ, f, N = p
...
# ╔═╡ 1f0df063-5cf5-4942-8231-30f3eac55fc9
begin
#fixed quantities
...
# ╔═╡ 676232bf-e5a4-4a75-af7b-38142bdd4240
begin 
	...
	t0 = (0,30+tc)
	t1 = (30+tc,30+tc+365)
	betainteractive0 = c0*q0
	betainteractive1 = c1*q1

	
	#first 30 days with sampling every day
	ret00p = ODEProblem(SEIRDmodel!, u0, tfirst, [beta0,ν,γ,f,N])
	ret00 = solve(ret00p,saveat=1)
	#with more sampling for model plotting
	ret0 = solve(ret00p,saveat=0.1)

	#Intervention 1 - no action
	tI1 = (0,365)
	retInt1p = ODEProblem(SEIRDmodel!, u0, tI1, [beta0,ν,γ,f,N])
	retInt1 = solve(retInt1p,saveat=0.1)

	#Intervention 2 - wait then respond
	retInt2ap = ODEProblem(SEIRDmodel!, u0, (0,35), [beta0,ν,γ,f,N])
	retInt2a = solve(retInt2ap,saveat=0.1)
	
	
	retInt2bp = ODEProblem(SEIRDmodel!, retInt2a.u[end], tafter, [beta1,ν,γ,f,N])
	retInt2b = solve(retInt2bp,saveat=0.1)


#Intervention 3 - large gathering
	retInt3ap = ODEProblem(SEIRDmodel!, u0, tfirst, [beta0,ν,γ,f,N])
	retInt3a = solve(retInt3ap,saveat=0.1)
	retInt3bp = ODEProblem(SEIRDmodel!, retInt3a.u[end], tgather, [betagather,ν,γ,f,N])
	retInt3b = solve(retInt3bp,saveat=0.1)
	retInt3cp = ODEProblem(SEIRDmodel!, retInt3b.u[end], taftergather, [beta1,ν,γ,f,N])
	retInt3c = solve(retInt3cp,saveat=0.1)

#Intervention 4 - immediately reduce close-contact interactions
	retInt4ap = ODEProblem(SEIRDmodel!, u0, tfirst, [beta0,ν,γ,f,N])
	retInt4a = solve(retInt4ap,saveat=0.1)
	retInt4bp = ODEProblem(SEIRDmodel!, retInt4a.u[end], tclose, [beta1,ν,γ,f,N])
	retInt4b = solve(retInt4bp,saveat=0.1)
	
#Intervention 5 - no new transmission after awareness reached
	retInt5ap = ODEProblem(SEIRDmodel!, u0, tfirst, [beta0,ν,γ,f,N])
	retInt5a = solve(retInt5ap,saveat=0.1)
	retInt5bp = ODEProblem(SEIRDmodel!, retInt5a.u[end], tclose, [0,ν,γ,f,N])
	retInt5b = solve(retInt5bp,saveat=0.1)

#Interactive
#from day 0 to changepoint
	ret00interactivep = ODEProblem(SEIRDmodel!, u0, (0,35), [betainteractive0,ν,γ,f,N])
	ret00interactive = solve(ret00interactivep,saveat=0.1)
	
	ret0p = ODEProblem(SEIRDmodel!, u0, t0, [betainteractive0,ν,γ,f,N])
	ret0 = solve(ret0p,saveat=0.1)
	ret1p = ODEProblem(SEIRDmodel!, ret0.u[end], t1, [betainteractive1,ν,γ,f,N])
	ret1 = solve(ret1p,saveat=0.1)

	#no action
	retNAinteractivep = ODEProblem(SEIRDmodel!, u0, (0,365), [betainteractive0,ν,γ,f,N])
	retNAinteractive = solve(retNAinteractivep,saveat=0.1)

# effective reproduction numbers
R_eff = [ret0[1,:] ./ (N/(betainteractive0/γ)); ret1[1,:] ./ (N/(betainteractive1/γ))]

R_effI1 = retInt1[1,:] ./ (N/(beta0/γ))
R_effI2 = [retInt2a[1,:] ./ (N/(beta0/γ)); retInt2b[1,:] ./ (N/(beta1/γ))]
R_effI3 = [retInt3a[1,:] ./ (N/(beta0/γ)); retInt3b[1,:] ./ (N/(betagather/γ)); retInt3c[1,:] ./ (N/(beta1/γ))]
R_effI4 = [retInt4a[1,:] ./ (N/(beta0/γ)); retInt4b[1,:] ./ (N/(beta1/γ))]
R_effI5 = [retInt5a[1,:] ./ (N/(beta0/γ)); retInt5b[1,:] ./ (N/(0/γ))]
	nothing
end

# ╔═╡ 49a62a4e-b0e4-4eba-90c2-567e5a7a03c0
begin
	scatter(ret00.t[1:28],ret00[5,1:28],labels="D",legend=:outertopright,fg_legend = :transparent, xlims = (16.5,30.5), ylims=(-1,maximum(ret00[5,1:28])+5),color="blue")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("Observed cumulative death in outbreak of unknown origin",titlefontsize=11)
end

# ╔═╡ ff04f968-08d3-4846-9341-eb6c44e896d2
begin
	plot(retInt1.t,retInt1[5,:],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,100),color="blue")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("1. Outbreak with no action",titlefontsize=11)
end

# ╔═╡ 20de2270-c7cb-40ec-bd5a-c4141fbfe89d
begin
	plot([retInt2a.t; retInt2b.t],[retInt2a[5,:];retInt2b[5,:]],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,100),color="blue")
	vline!([30,37],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("2. Outbreak response following delayed action",titlefontsize=11)
end

# ╔═╡ 8ff552c1-cf2a-46f0-b30d-f2469327ca09
begin
	plot([retInt3a.t; retInt3b.t; retInt3c.t],[retInt3a[5,:];retInt3b[5,:];retInt3c[5,:]],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,100),color="blue")
	vline!([30,33],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("3. Outbreak with large gathering",titlefontsize=11)
end

# ╔═╡ f021bb72-00ed-4d34-8555-37f30665ebe4
begin
	plot([retInt4a.t; retInt4b.t],[retInt4a[5,:];retInt4b[5,:]],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,100),color="blue")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("4. Outbreak with swift response",titlefontsize=11)
end

# ╔═╡ 967999f0-6668-43f6-a3ef-ac1f8fa502bb
begin
	plot([retInt5a.t; retInt5b.t],[retInt5a[5,:];retInt5b[5,:]],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,100),color="blue")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
	title!("5. Outbreak with no new infections",titlefontsize=11)
end

# ╔═╡ b8c78ff2-234a-40c3-b5e2-e4d369a88e43
begin
	l = @layout [a ; b]
	p1 = bar([1,2,3,4,5],[retInt1[5,end],retInt2b[5,end],retInt3c[5,end],retInt4b[5,end],retInt5b[5,end]], legend=false,ylabel="Total deaths")
	p2 = bar([1,2,3,4,5],[retInt1[5,end]-retInt1[5,end],retInt1[5,end]-retInt2b[5,end],retInt1[5,end]-retInt3c[5,end],retInt1[5,end]-retInt4b[5,end],retInt1[5,end]-retInt5b[5,end]],legend = false, ylabel = "Deaths averted", xlabel = "Intervention")
	plot(p1,p2,layout=l)
end

# ╔═╡ bec4ffc2-0b4c-4c8b-99d1-4c730d4fc7f3
begin
	ll = @layout[a b c;d e f]
	p11 = plot(retInt1.t,R_effI1,legend = false, xlims=(0,50),linewidth=3,ylabel=L"$\mathcal{R}_e (t)$")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 1")
	p12 = plot([retInt2a.t;retInt2b.t],R_effI2,legend = false, xlims=(0,50),linewidth=3)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 2")
	p13 = plot([retInt3a.t;retInt3b.t;retInt3c.t],R_effI3,legend = false, xlims=(0,50),linewidth=3)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 3")
	p14 = plot([retInt4a.t;retInt5b.t],R_effI4,legend = false, xlims=(0,50),linewidth=3,ylabel=L"$\mathcal{R}_e (t)$")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 4")
	p15 = plot([retInt5a.t;retInt5b.t],R_effI5,legend = false, xlims=(0,50),linewidth=3,xlabel="Time (days)")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 5")
	plot(p11,p12,p13,p14,p15,layout = ll)
end

# ╔═╡ f730f4e6-e4e9-4521-87bc-38f5427e976c
begin
	lll = @layout[a b c;d e f]
	p11l = plot(retInt1,legend=:topleft,labels=["S" "E" "I" "R" "D"], xlims=(0,50),linewidth=3,ylabel="Population",foreground_color_legend = nothing)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 1",titlefontsize=10)
	p12l = plot([retInt2a.t; retInt2b.t], [retInt2a[1,:]; retInt2b[1,:]],legend = false, xlims=(0,50),linewidth=3,labels="S")
	plot!([retInt2a.t; retInt2b.t], [retInt2a[2,:]; retInt2b[2,:]],linewidth=3,labels="E")
	plot!([retInt2a.t; retInt2b.t], [retInt2a[3,:]; retInt2b[3,:]],linewidth=3,labels="I")
	plot!([retInt2a.t; retInt2b.t], [retInt2a[4,:]; retInt2b[4,:]],linewidth=3,labels="R")
	plot!([retInt2a.t; retInt2b.t], [retInt2a[5,:]; retInt2b[5,:]],linewidth=3,labels="D")
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 2",titlefontsize=10)
	p13l = plot([retInt3a.t;retInt3b.t;retInt3c.t],[retInt3a[1,:]; retInt3b[1,:]; retInt3c[1,:]],legend = false, xlims=(0,50),linewidth=3)
	plot!([retInt3a.t; retInt3b.t; retInt3c.t], [retInt3a[2,:]; retInt3b[2,:]; retInt3c[2,:]],linewidth=3)
	plot!([retInt3a.t; retInt3b.t; retInt3c.t], [retInt3a[3,:]; retInt3b[3,:]; retInt3c[3,:]],linewidth=3)
	plot!([retInt3a.t; retInt3b.t; retInt3c.t], [retInt3a[4,:]; retInt3b[4,:]; retInt3c[4,:]],linewidth=3)
	plot!([retInt3a.t; retInt3b.t; retInt3c.t], [retInt3a[5,:]; retInt3b[5,:]; retInt3c[5,:]],linewidth=3)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 3",titlefontsize=10)
	p14l = plot([retInt4a.t; retInt4b.t], [retInt4a[1,:]; retInt4b[1,:]],legend = false, xlims=(0,50),linewidth=3,ylabel="Population")
	plot!([retInt4a.t; retInt4b.t], [retInt4a[2,:]; retInt4b[2,:]],linewidth=3)
	plot!([retInt4a.t; retInt4b.t], [retInt4a[3,:]; retInt4b[3,:]],linewidth=3)
	plot!([retInt4a.t; retInt4b.t], [retInt4a[4,:]; retInt4b[4,:]],linewidth=3)
	plot!([retInt4a.t; retInt4b.t], [retInt4a[5,:]; retInt4b[5,:]],linewidth=3)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 4",titlefontsize=10)
	p15l = plot([retInt5a.t; retInt5b.t], [retInt5a[1,:]; retInt5b[1,:]],legend = false, xlims=(0,50),linewidth=3,xlabel="Time (days)")
	plot!([retInt5a.t; retInt5b.t], [retInt5a[2,:]; retInt5b[2,:]],linewidth=3)
	plot!([retInt5a.t; retInt5b.t], [retInt5a[3,:]; retInt5b[3,:]],linewidth=3)
	plot!([retInt5a.t; retInt5b.t], [retInt5a[4,:]; retInt5b[4,:]],linewidth=3)
	plot!([retInt5a.t; retInt5b.t], [retInt5a[5,:]; retInt5b[5,:]],linewidth=3)
	vline!([30],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Intervention 5",titlefontsize=10)
	plot(p11l,p12l,p13l,p14l,p15l,layout = lll)
end

# ╔═╡ d947d0fd-58e4-4001-a544-e44fc7e7154b
begin
	plot([ret0.t; ret1.t], [ret0[1,:]; ret1[1,:]],legend=:right,labels="S", foreground_color_legend = nothing,xlims=(0,100),linewidth=3,xlabel="Time (days)")
	plot!([ret0.t; ret1.t], [ret0[2,:]; ret1[2,:]],linewidth=3,labels="E")
	plot!([ret0.t; ret1.t], [ret0[3,:]; ret1[3,:]],linewidth=3,labels="I")
	plot!([ret0.t; ret1.t], [ret0[4,:]; ret1[4,:]],linewidth=3,labels="R")
	plot!([ret0.t; ret1.t], [ret0[5,:]; ret1[5,:]],linewidth=3,labels="D")
	vline!([30 30+tc],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	title!("Interactive outbreak with intervention",titlefontsize=10)
end

# ╔═╡ 08d65bb1-6472-49c2-93ed-2bdedd2d27bd
begin
	plot([ret0.t; ret1.t],[ret0[5,:];ret1[5,:]],labels="D")
	scatter!(ret00.t[1:28],ret00[5,1:28],labels="D obs",legend=:outertopright,fg_legend = :transparent,xlims=(0,tc+50),color="blue")
	vline!([30 30+tc],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	xlabel!("Time (days)")
	ylabel!("Cumulative deaths")
end

# ╔═╡ b9a48fae-c7df-4009-b0d2-3384087ef74c
begin
	plot([ret0.t;ret1.t],R_eff,legend = false, xlims=(0,tc+50),linewidth=3, ylabel=L"\mathcal{R}_e (t)",xlabel="Time (days)")
	vline!([30 30+tc],linestyle=:dashdot,color="black",labels=false,linewidth=2)
	hline!([1],linestyle=:dashdot,color="black",labels=false,linewidth=2)
end

# ╔═╡ 523963d2-2173-43ca-bc85-9cdc9fa0a347
begin
	lf = @layout[a; b]
	pp = bar(["no action","intervention"],[retNAinteractive[5,end],ret1[5,end]],legend=false,ylabel="Cumulative deaths")
	qq =bar(["no action","intervention"],[retNAinteractive[5,end]-retNAinteractive[5,end],retNAinteractive[5,end]-ret1[5,end]],legend=false,ylabel="Deaths averted",xlabel="Intervention",ylim = (0,1.1*(retNAinteractive[5,end]-ret1[5,end])))
	scatter!(["no action","intervention"],[retNAinteractive[5,end]-retNAinteractive[5,end],retNAinteractive[5,end]-ret1[5,end]])
	plot(pp,qq,layout=lf)
end
...
manifest_format = "2.0"
project_hash = "c3b09fc01e5f627ded703ccdded554c2090a3774"
