...
# ╔═╡ c6abd784-a40b-4d2a-b8ff-17553732799e
begin
	using OrdinaryDiffEq, Plots, PlutoUI , PlutoTeachingTools
	import PlutoUI: combine
end

# ╔═╡ b5bb9c63-b040-47b6-8fef-ad710d62d187
md"""
...
## About this notebook
This notebook was designed and written using the Pluto.jl library in the Julia computer programming language. Julia is nearly 13 years old (much younger than other languages you may have heard of such as python, or R!) and was designed with scientific computing in mind. Stephen uses Julia in their research. You can read more about it here: [https://julialang.org/](https://julialang.org/)
...
# ╔═╡ ffff4061-e70a-4898-a0f1-2fb7a7e621b5
md"""
First, we will define the SIR model as a function which is designed to be compatible with the OrdinaryDiffEq package - which is used to simulate differential equations. In the below the rate of change of the state variables is defined by du, the state variables are represented by u, the parameters by p, and time (which we do use here) is given by t.
"""

# ╔═╡ e9d1fb35-5b90-4c5d-a401-b31fe7d839a5
...
# ╔═╡ 374691b3-31de-4d97-9971-482c9d4d83e2
md"""
...
# ╔═╡ 80673b93-0f7d-4e6a-a558-5d00d42696b0
begin
...
# ╔═╡ 35e344d8-20cd-4d0e-a079-302d7ee77e3c
function epi_input(vals::Vector,defaultval::Vector)
	
	return combine() do Child
		
		inputs = [
			md""" $(vals[aa]): $(
				Child(vals[aa], Slider(0.01:0.01:2.0, default = defaultval[aa], show_value = true))
			)"""
			
			for aa in 1:length(vals)
		]
...
# ╔═╡ 96d25240-dcbd-4e72-8a02-edc921f5f499
@bind epipars epi_input(["β", "γ"],[0.4,0.1])

# ╔═╡ ce9e7729-b388-4926-9186-e814bea8dd72
begin
		parameters = [epipars.β, epipars.γ]
		SIRproblem = ODEProblem(SIRmodel!, u0, tspan, parameters)
		solveSIR = solve(SIRproblem, saveat=tres)
		plot(solveSIR,labels=["S" "I" "R"],legend=:outertopright,fg_legend = :transparent)
		xlabel!("Time (days)")
		ylabel!("Population fraction")
		title!("SIR model simulation")
end

# ╔═╡ fddb4634-9c69-4729-b0ef-06baa3b1ffa0
question_box(md"""How many different categories of dynamic outcomes are there in the SIR model?
""")

# ╔═╡ daa24866-8126-4dd4-8d54-6cdb3935d156
md"""With some of the above parameter combinations the disease can take off and many (sometimes nearly all) people can become infected. But, this is not always the case. For some combinations of parameters the disease fails to spread much further than the intial infected population fraction. The SIR model can represent an epidemic, or a failed epidemic. Something it cannot represent is a scenario of a disease that becomes sustained in a population i.e., an endemic disease. We will return to this later.
...
# ╔═╡ 6fb2e6a1-d4dd-4113-846f-73457b73be91
question_box(md"""If $S+I+R=1$, we know $R$ are immune, and we assume that $I$ is approximately $0$, then what fraction of the population needs to be in $R$ such that the herd immunity threshold is reached?""")

# ╔═╡ faba6967-d9f9-44da-bdf1-e59489851ba4
md"""
# 2) Saving plots
Did you notice the interactive elements of the outbreak simulation above? If you go back to the plot above and right-click you will be able to open the image in a new tab - this may be useful if you want to compare different plots.

Go back and try this functionality!
"""

# ╔═╡ cfbb7793-d7f7-487d-8277-39bb3847307e
md"""# 3) Endemic outbreaks and waning immunity
...
# ╔═╡ 46460d18-2682-4e6e-b6cb-e71dcbe3e722
# The SIRS model differential equations.
function SIRSmodel!(du,u,p,t)
    S, I, R = u
    β, γ, α = p
...
# ╔═╡ 8b42eb62-5c27-43dd-8c93-efa8debce289
md"""
			#### Epidemiological parameters for the SIRS model
			Transmission rate, β = 0.4 /day (note, this is fixed)

			Recovery rate, γ = 0.1/day (note, this is fixed)

			Average immunity duration, 1/α: $(@bind alpha Slider(1:1000,default=365, show_value=true))

			Choose length to time to view simulation output for
			$(@bind timescale Select([200 => "200 days", 1040 => "3 years"]))
			
			"""

# ╔═╡ d3a4cde0-f1ba-4a1e-b63c-243da1af95da
begin
		beta = 0.4
		gamma = 0.1
		parametersS = [beta, gamma, 1/alpha]
		tspanS = (0,timescale)
		SIRSproblem = ODEProblem(SIRSmodel!, u0, tspanS, parametersS)
		solveSIRS = solve(SIRSproblem, saveat=tres)
		plot(solveSIRS,labels=["S" "I" "R"],legend=:outertopright,fg_legend = :transparent)
		xlabel!("Time (days)")
		ylabel!("Population fraction")
		title!("SIRS model simulation")
end

# ╔═╡ 4b04ce3c-cbce-4acf-a030-7eec4aa9abb8
question_box(md"What are the differences in dynamics between the SIR and the SIRS model? Hint: go back to the SIR model and match the parameters used here in the SIRS model output.")

# ╔═╡ 1dd490bb-48d4-4813-bb3f-7982f421a3a4
md"""
    ...
    **Q:** what happens during an infection event? **A:** The number of susceptibles decreases by 1, and the number of infectious individuals increases by 1.   So, what happens during a recovery event?
    ...
    The simulation technique we use here is termed the Gillespie algorithm (aka stochastic simulation algorithm): [https://en.wikipedia.org/wiki/Gillespie_algorithm](https://en.wikipedia.org/wiki/Gillespie_algorithm)

    Our implementation of the Gillespie algorithm for the SIR model is in the next code block:
    """

# ╔═╡ 54000061-f182-4e9a-82b6-6b9229f093a3
function StochasticSIR(InitS, InitI, InitR, β, γ)
	Pop = InitS+InitI+InitR
    ...
    append!(time, 0) #at intial time 0
    append!(Slist,InitS) #at intial time 0 S will have SInit individuals
    append!(Ilist,InitI)  #at intial time 0 I will have IInit individuals
    append!(Rlist,InitR)  #at intial time 0 R will have RInit individuals
    Itot = InitI
    #start the reaction loop
	aa = 0
    while Itot>0 #while more than one infectious individual in the population
        aa+=1
		r = [β*Slist[aa]*Ilist[aa]/Pop, γ*Ilist[aa]] #rates of transmission and recovery in the population
        if sum(r.<0)>0 # a check to make sure rates are not negative, and if yes to exit.
        	...
        	r_prob = cumsum(r)./sumr;
        	tau = 1/sumr * log(1/rand()); #time to next event
        	append!(time,time[aa]+tau); #access the last element in the current list of time, and add tau
        	choose = rand() #random value between 0 and 1
        	index = findall(choose.>r_prob) #is the random value greater than the proportional rates?
        	if length(index)==1
            	Itot = Itot-1 #recovery event
            	append!(Slist,Slist[aa])
				append!(Ilist,Ilist[aa] - 1) 
				append!(Rlist,Rlist[aa] + 1) 
            else
            	Itot = Itot+1 #transmission event
				append!(Slist,Slist[aa] - 1)
				append!(Ilist,Ilist[aa] + 1) 
				append!(Rlist,Rlist[aa]) 
			end
		end
	end
...
# ╔═╡ c786d037-19b9-4834-b4ee-0766ecd9d9cc
md"""
			...
			$(@bind popsize Select([10000 => "10,000", 500 => "500", 100 => "100"]))

			When you are ready press this button to run the simulation: 
			$(@bind go Button("click to run"))
			"""

# ╔═╡ d6626598-9248-471d-a6a1-bb4a571caddc
begin
	...
	stochsim = StochasticSIR(S0, I0, R0, 0.4, 0.1)

	plot(stochsim[1],stochsim[2],labels="S",legend=:outertopright,fg_legend = :transparent)
	plot!(stochsim[1],stochsim[3],labels="I")
	plot!(stochsim[1],stochsim[4],labels="R")
	xlabel!("Time (days)")
	ylabel!("Population fraction")
	title!("SIRS model simulation")
...
# ╔═╡ 5f2304a8-2f21-492f-9ded-3e1cd9ab06a9
question_box(md"""Do the dynamics match up to the SIR model in section 1? Are they similar in their qualitative behavior, and the timing of epidemic peaks? How does this change with population size? """)

# ╔═╡ fd27ad25-33b7-4b56-8bc0-94b3824cffd4
almost(md"""
...
# ╔═╡ 77f3f848-8707-4269-b2d3-03d722f0e320
md"""
...
# ╔═╡ 7febab47-4a3e-4a17-bed9-65200c095899
@bind ensemble html"""
<div>
...
# ╔═╡ 4dc6a505-9957-4cda-ac1c-8f10ba9276db
begin
	if ensemble>0
		listofR = []
		I0 = 1
		...
		R0 = 0
		for aa = 1:100
			stochsims = StochasticSIR(S0, I0, R0, 0.4, 0.1)
			append!(listofR,stochsims[4][end])
		end

		histogram(listofR,bins = 100, legend=false)
		xlabel!("Outbreak size")
		ylabel!("Number of outbreaks")
		title!("Outbreak size distributions from 100 simulations")
	end
end

# ╔═╡ 297c6705-fb98-426e-90ef-8661c5a569a5
correct(md"""# You made it!
