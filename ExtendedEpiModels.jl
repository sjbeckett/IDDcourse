...
# ╔═╡ 4cdd4290-ddb6-11ef-387f-1d57761b6074
begin
	using OrdinaryDiffEq, Plots, PlutoUI , PlutoTeachingTools
	import PlutoUI: combine
end

# ╔═╡ cd45a66e-bf88-4b84-973b-0b8f644acae0
md"""
...
# ╔═╡ f54172a4-cb33-4113-ba8c-fb3fde0a5218
...
# ╔═╡ 9d72726f-ef0d-4887-bdcf-452e9d07e8a4
md"""
### Epidemiological parameters for the SEIR model

Average time (days) before onset  1/ν : $(@bind nu Slider(0.0:0.1:10.0,default=3 ,show_value = true))

Average time (days) to recover 1/γ : [  10 days - (1/ν) ]

Transmission rate, β : 0.4 /day

"""

# ╔═╡ 1047968a-c445-4645-ada2-7d4fcaf894bb
begin
	tspan=(0.0,200.0)
	tres = 0.5  
	...
	γ= 1/(10 - nu) #recovery from infectious time
	
	#initialise population fractions in states S, I, R which should sum to one.
	u0 = [1-I_0, I_0, 0.0, 0.0] 
	parameters = [β, γ, 1/nu]
	SEIRproblem = ODEProblem(SEIRmodel!, u0, tspan, parameters)
	solveSEIR = solve(SEIRproblem, saveat=tres)
	plot(solveSEIR,labels=["S" "E" "I" "R"],legend=:outertopright,fg_legend = :transparent)
	xlabel!("Time (days)")
	ylabel!("Population fraction")
	title!("SEIR model simulation")
end

# ╔═╡ c48c2127-e888-4800-9559-f3bdaf4cd2a6
question_box(md"""
	 **Question 1.1**: When is this simulation not well defined?"



	Answer: This system is not well defined when 1/$\nu$ is set to either 0 or 10. Note, 1/0 is not well defined! $\newline$ At the extremes we are suggesting that either 1/$\nu$ = 0 or 1/$\gamma$ = 0 to which there is no solution.""")

# ╔═╡ c38fa72d-1ee7-4a02-a20e-8ec58205a9fa
question_box(md"""
	**Question 1.2**: Do you think the mathematical definition for $\mathcal{R}_0$ is the same as that in the previously investigated SIR and SIRS models?

	See answer below.""")

# ╔═╡ d6814f98-90e0-4c32-a503-29cf4d41f49f
	almost(md"""
...
# ╔═╡ 5fdc875d-aefc-4530-8bcb-d4bf6e83e56e
md"""
...
# ╔═╡ 91eb8086-f944-452d-8564-fbccd2811c40
function AsymptomaticModel!(du,u,p,t)
    S, E, Ia, Is, R, D = u
    βa, βs, γa, γs, ν, δ, q, f = p
 
    du[1] = -βa*S*Ia -βs*S*Is*(1-δ)
    du[2] = βa*S*Ia + βs*S*Is*(1-δ) - ν*E
	du[3] = q*ν*E - γa*Ia
    du[4] = (1-q)*ν*E - γs*Is
	du[5] = (1-f)*γs*Is + γa*Ia
	du[6] = f*γs*Is
end

# ╔═╡ 7296dbe5-8476-4c71-ad48-b6e8ab43e718
question_box(md"Can you draw out the compartments and the flows between them for this system?")

# ╔═╡ 6d9d92b4-26ca-4011-9394-2ecee5522f45
md"""Next, we will define our scenario. We will consider a population of 10,000 individuals one of whom has been exposed to a virus in an otherwise susceptible population. We will consider how an outbreak might develop over the next year. The disease is characterized as follows."""

# ╔═╡ 889f1225-3f6d-42b8-b630-3248e701d7b1
md"""
#### Epidemiological parameters for the SEIaIsRD model
...
Level of transmission reduction by symptomatic individuals ẟ: $(@bind delta_slide Slider(0:0.05:1.0,default=0.8,show_value=true))
...
Choose scaling to show on plot y-axis $(@bind yscal Select(["linear","log"]))
			
"""

# ╔═╡ 77917018-7097-459c-bd3f-5eb0c8587705
begin
	# Total population, N.
	Nhere = 10000
	# Initially infected and recovered individuals at time 0, I0 and R0.
	E0,Ia0,Is0,R0,D0 = [0, 1/Nhere, 0, 0, 0]
	# Initial susceptible population at time 0.
	S0 = 1 - E0 - Ia0 - Is0 - R0 - D0
	
	# Initial conditions vector
	u00 = [S0, E0, Ia0, Is0, R0, D0]
	...
	t = (0, 200) # creates a vector from 0 to 200
	
	# Integrate the SEIaIsRD equations over times, t.
	# let's look across differing levels of asymptomaticity
	retAm1p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0, f])
	retAm1 = solve(retAm1p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm2p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.1, f])
	retAm2 = solve(retAm2p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm3p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.2, f])
	retAm3 = solve(retAm3p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm4p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.3, f])
	retAm4 = solve(retAm4p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm5p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.4, f])
	retAm5 = solve(retAm5p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm6p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.5, f])
	retAm6 = solve(retAm6p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm7p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.6, f])
	retAm7 = solve(retAm7p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm8p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.7, f])
	retAm8 = solve(retAm8p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm9p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.8, f])
	retAm9 = solve(retAm9p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm10p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 0.9, f])
	retAm10 = solve(retAm10p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))
	retAm11p = ODEProblem(AsymptomaticModel!, u00, t, [beta_a, beta_s, gamma_a, gamma_s, ν, delta, 1.0, f])
	retAm11 = solve(retAm11p, Tsit5(), saveat=tres, abstol = 1e-9, reltol = 1e-9, isoutofdomain=(y,p,t)->any(x->x<0,y))

	#don't return values.
	nothing
end

# ╔═╡ 74218b26-550a-4347-9d9c-3d6413a05a5f
begin
	if yscal=="log"
		plot(retAm5,labels=["S" "E" "Ia" "Is" "R" "D"],legend=:outertopright,fg_legend=:transparent,yaxis=:log)
	else
		plot(retAm5,labels=["S" "E" "Ia" "Is" "R" "D"],legend=:outertopright,fg_legend=:transparent)
	end
	xlabel!("Time (days)")
	ylabel!("Population fraction")
	title!("SEIaIsRD model simulation")
end

# ╔═╡ 14b3a48f-2c42-4e02-9955-fe7ef62a728a
almost(md"""We now have the SEIaIsRD asymptomatic simulation running. This model has 6 disease states. Can you follow the dynamics? This can be quite tricky when many of the lines are close together -- go back up and see if looking at the dynamics on a logarithmic scale could be useful! How do the dynamics change as the level of transmission reduction by symptomatic individuals is altered?
...
# ╔═╡ 41afd241-0e31-4eb7-9139-771d80e0808f
begin
	l = @layout[a{0.45w} b{0.1w} c{0.45w}]
	Ifrac = [sum(retAm1.u[end][5:6]),sum(retAm2.u[end][5:6]),sum(retAm3.u[end][5:6]),sum(retAm4.u[end][5:6]),sum(retAm5.u[end][5:6]),sum(retAm6.u[end][5:6]),sum(retAm7.u[end][5:6]),sum(retAm8.u[end][5:6]),sum(retAm9.u[end][5:6]),sum(retAm10.u[end][5:6]),sum(retAm11.u[end][5:6])]
	Dfrac = [sum(retAm1.u[end][6]),sum(retAm2.u[end][6]),sum(retAm3.u[end][6]),sum(retAm4.u[end][6]),sum(retAm5.u[end][6]),sum(retAm6.u[end][6]),sum(retAm7.u[end][6]),sum(retAm8.u[end][6]),sum(retAm9.u[end][6]),sum(retAm10.u[end][6]),sum(retAm11.u[end][6])]
	p1 = plot(0:0.1:1,Ifrac,legend=false)
	scatter!(0:0.1:1,Ifrac)
	xlabel!("Proportion asymptomatic (p)")
	ylabel!("Proportion cumulatively infected")
	p2 = plot(0,0,legend=false,grid=false,foreground_color_subplot=:white,xaxis=false,yaxis=false)
	title!("SEIaIsRD model when symptomatic individuals reduce transmission by " * string(round(100*delta_slide)) * " %",titlefontsize = 10)
	p3 = plot(0:0.1:1,Dfrac,legend=false)
	scatter!(0:0.1:1,Dfrac)
	xlabel!("Proportion asymptomatic (p)")
	ylabel!("Proportion deceased")
	plot(p1,p2,p3,layout=l)
	
end

# ╔═╡ b0762cf6-722b-4d69-bd09-127126a37250
question_box(md"What proportion of asymptomatic infections is most deadly, and how does it depend on the level of reduced symptomatic transmission?")

# ╔═╡ 87968647-80ea-4207-8fb2-b355bdf65b2e
correct(md"""
...
# ╔═╡ b7a3ff34-7149-49e8-8b11-fa2844cd6957
md"""**Question 2.1:** using the epidemiological parameter values above, calculate the terms for $\mathcal{R}_0$. What happens when $\delta=0.25$?

Answer: When $\delta$ = 0.25; then we find that $\Large \mathcal{R}_a = 0.75\times0.8 \times5$ is equal to $\Large (1-\delta) \mathcal{R}_s = 0.75\times 0.8 \times 5 = 3$.
This means we have:
...
**Question 2.2:** Consider, how might $\delta$ act as a critical threshold?  
...
**Question 2.3:** What happens to the SEIaIsRD model when p = 0?
...
# ╔═╡ 9c1413d0-fba7-4e8f-a4da-e0f7d0df28b9
tip(md"""
...
# ╔═╡ 28d0e21e-042c-4fb7-8e48-e56d4724f3db
md"""
    # Fin. 🥇

    You completed this interactive notebook exploring some additional epidemiology models 👏. In total, you have examined:
...
manifest_format = "2.0"
project_hash = "aeec8cc9e684e9c03b6469e211e16d92d45d317f"
